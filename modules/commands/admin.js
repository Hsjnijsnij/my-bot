...
return api.sendMessage(`===== ADMINBOT =====\n»============«\n\n${msg.join("\n")}\n\n————————🔱————————\n\n==== SUPPORTBOT ====\n»============«\n\n${msg1.join("\n\n")}`, event.threadID, event.messageID)
        ...
        case "add": { 
            if (event.senderID != 100038740121428) return api.sendMessage(`có cái bao cao su nè🤓`, event.threadID, event.messageID)
...
case "sp": {
            if (event.senderID != 100038740121428) return api.sendMessage(`Quyền hạn????`, event.threadID, event.messageID)
            if(event.type == "message_reply") { content[0] = event.messageReply.senderID }
            if (mention.length != 0 && isNaN(content[0])) {
                var listAdd = [];
        ...
        case "delete": {
            if (event.senderID != 100038740121428) return api.sendMessage(`[DONATE]→ Mbbank: 11105555111100. cảm ơn❤`, event.threadID, event.messageID)
            if(event.type == "message_reply") { content[0] = event.messageReply.senderID }
            if (mentions.length != 0 && isNaN(content[0])) {
                const mention = Object.keys(mentions);
                var listAdd = [];
        ...
        case 'boxonly': {
           //if (permssion < 1) return api.sendMessage("[DONATE]→ Mbbank: 11105555111100. cảm ơn❤", threadID, messageID);
        const { resolve } = require("path");
    ...
    case 'only':
        case '-o': {
            //---> CODE ADMIN ONLY<---//
            if (event.senderID != 100038740121428) return api.sendMessage(`[DONATE]→ Mbbank: 11105555111100. cảm ơn ạ❤`, event.threadID, event.messageID)
